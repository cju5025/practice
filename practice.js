...
//     const parsedReportArray = reportArray.map(e => parseInt(e, 10))
//     for (let i = 0; i < parsedReportArray.length; i++){
//         for (let j = i + 1; j < parsedReportArray.length; j++){
//              if (parsedReportArray[i] + parsedReportArray[j] === 2020) {
//                  return parsedReportArray[i] * parsedReportArray[j]
//              }
//         }
//     }
// }
...
// function expenseReportTrips (report) {
//     const reportArray = report.split('\n')
//     const parsedReportArray = reportArray.map(e => parseInt(e, 10))
//     for (let i = 0; i < parsedReportArray.length; i++){
//         for (let j = i + 1; j < parsedReportArray.length; j++){
//             for (let k = j + 1; k < parsedReportArray.length; k++){
//                 if (parsedReportArray[i] + parsedReportArray[j] + parsedReportArray[k] === 2020) {
//                     return parsedReportArray[i] * parsedReportArray[j] * parsedReportArray[k]
//                 }
//             }
//         }
//     }
...
// console.log(expenseReportTrips(expenses))
...
function passwordDebugger(passwords){
    const validPasswords = 0
    const arrayOfAllPasswords = passwords.split('\n')

    for (let i = 0; i < arrayOfAllPasswords.length; i++){
        const splitInput = arrayOfAllPasswords[i].split(':')
        const password = splitInput[1]
        const passwordConditions = splitInput[0]
        const conditionCharacter = passwordConditions.substring(passwordConditions.length - 2)
        const conditionCharacterValues = passwordConditions.substring(0, passwordConditions.length - 2)
        const minCharacterValue = conditionCharacterValues.split('-')[0]
        const maxCharacterValue = conditionCharacterValues.split('-')[1]
        const passwordArray = password.split('')
        let charCounter = 0

        for (let j = 0; j < passwordArray.length; j++){
            passwordArray[j] === conditionCharacter ? charCounter += 1 : null
        }
    }
}

//console.log(passwordDebugger(passwords))
