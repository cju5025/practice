...
function passwordDebugger(passwords){
    ...
    for (let i = 0; i < arrayOfAllPasswords.length; i++){
        ...
        const conditionCharacter = passwordConditions.substring(passwordConditions.length - 1)
        const conditionCharacterValues = passwordConditions.substring(0, passwordConditions.length - 2)
        const firstCharacterPlacementValue = conditionCharacterValues.split('-')[0]
        const secondCharacterPlacementValue = conditionCharacterValues.split('-')[1]
        const passwordArray = password.split('')

            if (passwordArray[firstCharacterPlacementValue] === conditionCharacter
                 && 
                 passwordArray[secondCharacterPlacementValue] !== conditionCharacter) {
                     validPasswords += 1
                 }  else if (passwordArray[firstCharacterPlacementValue] !== conditionCharacter 
                &&
                passwordArray[secondCharacterPlacementValue] === conditionCharacter) {
                    validPasswords += 1
                 }
